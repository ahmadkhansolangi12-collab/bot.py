...
# TEST MODE (True = only test message, False = real emails)
TEST_MODE = True
...
def get_fancy_time():
    ...
    time_str = time_str.replace("AM", "𝐀𝐌").replace("PM", "𝐏𝐌")
...
def send_telegram(service, logo, otp):
    ...
    message = f"""
🔥 {logo} {fancy_text(service.upper())} OTP 🔥

📩 OTP: {fancy_number(otp)}
⏰ TIME: {get_fancy_time()}
...
def test_send():
    ...
    send_telegram("Test Service", "🧪", "123456")
